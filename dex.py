...
class UniswapForkGraph:
    ...
    def _q_get_graph_ql_token_or_pair_info(cls, token_or_pair_address: str):
        ...
        return {
            ...
            'variables': {'token_address': token_or_pair_address.lower()},  # case sensitive
            'query': query
        }
    ...
    @classmethod
    def _q_get_graph_ql_price_payload(cls, token_address: str, block_nums: Iterable[int]) -> Dict:
        """
        ...
        query = "query blocks($token_id: ID!) {{{0}}}".format(
            "".join(
                (
                    cls._q_get_graph_ql_token_price_block(y) for y in block_nums
                )
            )
        )
        return {
    ...
    @classmethod
    def _q_get_graph_ql_lp_pair_payload(cls, lp_pair_address: str, tx_start_ts: int, tx_end_ts: int) -> Dict:
    ...
    def _get_graph_ql_pair_data(self, lp_pair_address: str, tx_start_ts: int, tx_end_ts: int) -> Dict[str, Dict]:
        if tx_start_ts > tx_end_ts:
            raise ValueError(
                "Invalid timestamp range for liquidity position price query! Date: ({0}, {1})".format(
                    tx_start_ts,
                    tx_end_ts
                )
            )
        ...
        return self._graph_ql_request(
            self._q_get_graph_ql_lp_pair_payload(lp_pair_address, tx_start_ts, tx_end_ts)
        )
    ...
    def _graph_ql_price_result_to_block_price_timeseries(
            cls,
            block_nums: Iterable[int],
            price_query_data: Dict
    ) -> Dict[int, Decimal]:
        ts = {}
    ...
    def get_token_price_by_block_timeseries(
        ...
        return self._graph_ql_price_result_to_block_price_timeseries(
            block_nums,
            self._get_graph_ql_price_query_data(token_address, block_nums)
        )
    ...
    def get_lp_token_price_by_block_timeseries(
        ...
        price_query_data = self._get_graph_ql_pair_data(lp_token_address, min_ts, max_ts)
        price_blocks: List[Dict] = price_query_data['pair']['liquidityPositionSnapshots']
        ts = {}

        for block_num in block_nums:
            price_block = price_blocks[self._get_best_block_idx(block_num, price_blocks)]
    ...
    def _get_best_block_idx(find_block: int, price_blocks: List[Dict]) -> int:
        ...
        while l < u:
            ...
            best_block_info = (
                    error < best_block_info[1] and (c, error) or best_block_info
            )
