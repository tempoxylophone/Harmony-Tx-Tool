...
class KoinlyReportCreator:
    ...
    def get_csv_from_transactions(self, events: Sequence[HarmonyEVMTransaction]) -> str:
        # remove out of range
        events = [
            x for x in events if self.dt_lb.timestamp() <= x.timestamp <= self.dt_ub.timestamp()
        ]
        ...
        return (
                self.get_csv_row_header() +
                "".join(self.to_csv_row(tx) for tx in events)
        )
    ...
    def to_csv_row(self, tx: HarmonyEVMTransaction) -> str:
        if (
                self.omit_tracked_fiat_prices and
                self.currency_is_tracked(tx.coinType.universal_symbol)
        ):
            fiat_value = ''
        else:
