...
def checkTransactions(txs, account, startDate, endDate, network, alreadyComplete=0):
    ...
    for txn in txs:
        ...
        try:
            ...
            print("Got invalid transaction {0} {1}".format(tx, str(err)))
            continue

        # TODO deduct gas from cost basis
        txFee = Web3.fromWei(result["gasPrice"], "ether") * receipt["gasUsed"]
        feeValue = (
            prices.priceLookup(timestamp, contracts.getNativeToken(network)) * txFee
        )
        ...
        if receipt["status"] == 1:
            ...
            if "Quest" in action and result["input"] != "0x":
                ...
                if "Pet" in action:
                    results = extractAuctionResults(
                        w3, tx, account, timestamp, receipt, "pet"
                    )
                else:
                    results = extractAuctionResults(
                        w3, tx, account, timestamp, receipt, "hero"
                    )
                ...
                results = extractAuctionResults(
                    w3, tx, account, timestamp, receipt, "land"
                )
                if results != None and results[0] != None:
                    results[0].fiatFeeValue = feeValue
                    events_map["tavern"].append(results[0])
                ...
                if results != None:
                    ...
                    logging.error(
                        "Error: Failed to parse a swap result. {0}".format("not needed")
                    )
            elif "Gardener" in action:
                results = extractGardenerResults(
                    w3, tx, account, timestamp, receipt, network
                )
                if results != None and len(results) > 0:
                    ...
                    logging.error(
                        "Error: Failed to parse a Gardener LP Pool result. tx {0}".format(
                            tx
                        )
                    )
                ...
                eventsFound = True
                results = extractLendingResults(
                    w3, tx, account, timestamp, receipt, network, value
                )
                if results != None:
                    if results[0] != None:
                        results[0].fiatFeeValue = feeValue
                ...
                for item in results:
                    ...
                    for item in results:
                        if recipientAccount not in ["", item.address]:
                            logging.error(
                                "Bailing from multi recipient payment {0}".format(tx)
                            )
                            continue
            ...
            elif result["input"] != "0x" and (
                "xJewels" in action or "xCrystal" in action
            ):
                ...
                if results != None:
                    ...
                    contract = w3.eth.contract(
                        address="0xA9cE83507D872C5e1273E745aBcfDa849DAA654F", abi=ABI
                    )
                    decoded_logs = contract.events.Transfer().processReceipt(
                ...
                logging.debug(
                    "Vendor activity: {0}".format(str(receipt["logs"][0]["address"]))
                )
                results = extractSwapResults(w3, tx, account, timestamp, receipt)
                ...
                if results != None:
                        ...
                        if results[0] in heroCrystals:
                            ...
                            for r in heroCrystals[results[0]][2]:
                                if r != None:
                                    r.itemID = results[1]
                            events = [
                                heroCrystals[results[0]][2][0],
                                heroCrystals[results[0]][2][1],
                            ]
                    ...
                    if (
                        type(results[1]) != int
                        and len(results[1]) > 2
                        and results[1][2] != None
                    ):
                        eventsFound = True
                else:
                    logging.info(
                        "Error: Failed to parse a summon result. {0}".format(tx)
                    )
                ...
                if results != None:
                    if results[1] != None and results[1].event == "crack":
                        ...
                        if results[0] in petEggs:
                            ...
                            eggToken = constants.EGG_TYPES[
                                petEggs[results[0]][2].itemID
                            ]
                            petEggs[results[0]][2].itemID = results[1].itemID
                            hatchCosts = 0
                            costList = "costs"
                            for k, v in petEggs[results[0]][6].items():
                                costList = "{0},{1} {2}".format(
                                    costList, v, contracts.getAddressName(k)
                                )
                                priceEach = prices.priceLookup(timestamp, k)
                                hatchCosts += priceEach * v
                                logging.info(
                                    "Adding cost {0} {1} for {2}".format(
                                        v, contracts.getAddressName(k), priceEach
                                    )
                                )
                    ...
                    logging.info(
                        "Error: Failed to parse a hatching result. {0}".format(tx)
                    )
                ...
                if results != None:
                    ...
                    logging.info(
                        "Error: Failed to parse a meditation result. {0}".format(tx)
                    )
                ...
                contract = w3.eth.contract(
                    address="0xdF0Bf714e80F5e6C994F16B05b7fFcbCB83b89e9", abi=ABI
                )
                decoded_logs = contract.events.Gen0Purchase().processReceipt(
                    receipt, errors=DISCARD
                )
                heroId = 0
                purchasePrice = 0
                ...
                for log in decoded_logs:
                    logging.info(
                        "Purchase Gen0 Crystal id {0} price {1}".format(
                            log["args"]["crystalId"], log["args"]["purchasePrice"]
                        )
                    )
                    crystalId = log["args"]["crystalId"]
                    purchasePrice = log["args"]["purchasePrice"]
                decoded_logs = contract.events.CrystalOpen().processReceipt(
                    receipt, errors=DISCARD
                )
                for log in decoded_logs:
                    logging.info(
                        "Open Gen0 Crystal id {0} heroid {1}".format(
                            log["args"]["crystalId"], log["args"]["heroId"]
                        )
                    )
                    crystalId = log["args"]["crystalId"]
                ...
                if crystalId != 0:
                    ...
                    if log["args"]["crystalId"] not in heroCrystals:
                        heroCrystals[log["args"]["crystalId"]] = [
                            tx,
                            timestamp,
                            purchasePrice,
                            heroId,
                        ...
                        heroCrystals[log["args"]["crystalId"]][2] = max(
                            purchasePrice, heroCrystals[log["args"]["crystalId"]][2]
                        )
                        heroCrystals[log["args"]["crystalId"]][3] = max(
                            heroId, heroCrystals[log["args"]["crystalId"]][3]
                        )
                        if (
                            heroCrystals[log["args"]["crystalId"]][2] > 0
                            and heroCrystals[log["args"]["crystalId"]][3] > 0
                        ):
                            heroPrice = Web3.fromWei(
                                heroCrystals[log["args"]["crystalId"]][2], "ether"
                            )
                            r = records.TavernTransaction(
                                tx,
                                "hero",
                                heroCrystals[log["args"]["crystalId"]][3],
                                "purchase",
                                timestamp,
                            ...
                            r.fiatAmount = (
                                prices.priceLookup(timestamp, "defi-kingdoms")
                                * r.coinCost
                            )
                ...
                results = extractPotionResults(
                    w3, tx, account, timestamp, receipt, result["input"]
                )
                eventsFound = True
                if results != None:
                    results.fiatFeeValue = feeValue
...
def lookupEvent(fm, to, account) -> str:
    ...
    for log in decoded_logs:
        ...
        if sentAmount > 0 and rcvdAmount > 0:
            if sentToken in [
                ...
                r = records.BankTransaction(
                    txn,
                    timestamp,
                    "withdraw",
                    rcvdAmount / sentAmount,
                    rcvdToken,
                    rcvdAmount,
                )
            else:
...
def extractGardenerResults(w3, txn, account, timestamp, receipt, network):
    ...
    contract = w3.eth.contract(
        address="0xDB30643c71aC9e2122cA0341ED77d09D5f99F924", abi=ABI
    )
    events = []
    decoded_logs = contract.events.SendGovernanceTokenReward().processReceipt(
        receipt, errors=DISCARD
    )
    for log in decoded_logs:
        receivedAmount = Web3.fromWei(log["args"]["amount"], "ether")
        lockedAmount = Web3.fromWei(log["args"]["lockAmount"], "ether")
        r = records.GardenerTransaction(
            txn, timestamp, "staking-reward", powerToken, receivedAmount - lockedAmount
        )
        rl = records.GardenerTransaction(
            txn, timestamp, "staking-reward-locked", powerToken, lockedAmount
        )
    ...
    contract = w3.eth.contract(
        address="0x72Cb10C6bfA5624dD07Ef608027E366bd690048F", abi=ABI
    )
    decoded_logs = contract.events.Transfer().processReceipt(receipt, errors=DISCARD)
    gardenEvent = ""
    gardenToken = ""
    ...
    for log in decoded_logs:
        ...
        if (
            "to" in log["args"]
            and "from" in log["args"]
            and log["address"] in contracts.address_map
            and "LP Token" in contracts.address_map[log["address"]]
        ):
            if log["args"]["to"] == account:
    ...
    if gardenAmount > 0:
        r = records.GardenerTransaction(
            txn, timestamp, gardenEvent, gardenToken, gardenAmount
        )
        events.append(r)
...
def extractFarmResults(w3, txn, account, timestamp, receipt):
    ...
    contract = w3.eth.contract(
        address="0x1f806f7C8dED893fd3caE279191ad7Aa3798E928", abi=ABI
    )
    events = []
    decoded_logs = contract.events.Transfer().processReceipt(receipt, errors=DISCARD)
    farmEvent = ""
    ...
    for log in decoded_logs:
        ...
        if "to" in log["args"] and "from" in log["args"]:
            if (
                "Tranq" in contracts.getAddressName(log["address"])
                or "Pangolin LP" in contracts.getAddressName(log["address"])
            ) and log["args"]["to"] == account:
                farmEvent = "withdraw"
            ...
            if log["args"]["to"] == account:
                ...
                "Tranq" in contracts.getAddressName(log["address"])
                or "Pangolin LP" in contracts.getAddressName(log["address"])
            ) and log["args"]["from"] == account:
                farmEvent = "deposit"
                farmToken = log["address"]
                farmAmount = Web3.fromWei(log["args"]["value"], "ether")
    if farmAmount > 0:
        r = records.GardenerTransaction(
            txn, timestamp, farmEvent, farmToken, farmAmount
        )
        events.append(r)
    if rewardAmount > 0:
        rr = records.GardenerTransaction(
            txn, timestamp, rewardEvent, rewardToken, rewardAmount
        )
...
def extractSwapResults(w3, txn, account, timestamp, receipt):
    ...
    contract = w3.eth.contract(
        address="0xcF664087a5bB0237a0BAd6742852ec6c8d69A27a", abi=ABI
    )
    ...
    for log in decoded_logs:
        ...
        if "to" in log["args"] and "from" in log["args"]:
            ...
            if log["args"]["to"] == account or (
                log["address"] == "0xCCb93dABD71c8Dad03Fc4CE5559dC3D89F67a260"
                and log["args"]["from"] == "0x3C351E1afdd1b1BC44e931E12D4E05D6125eaeCa"
                and log["args"]["to"] == "0x0000000000000000000000000000000000000000"
            ):
                rcvdToken.append(log["address"])
                rcvdAmount.append(
                    contracts.valueFromWei(log["args"]["value"], log["address"])
                )
            elif log["args"]["from"] == account or (
                log["address"] == "0xCCb93dABD71c8Dad03Fc4CE5559dC3D89F67a260"
                and log["args"]["to"] == "0x3C351E1afdd1b1BC44e931E12D4E05D6125eaeCa"
                and log["args"]["from"] == "0x0000000000000000000000000000000000000000"
            ):
                ...
                logging.debug(
                    "ignored swap log {0} to {1} not involving account".format(
        ...
        if "wad" in log["args"]:
            if log["event"] == "Withdrawal":
                rcvdToken.append(log["address"])
                rcvdAmount.append(
                    contracts.valueFromWei(log["args"]["wad"], log["address"])
                )
    ...
    if len(rcvdAmount) > 0 and rcvdAmount[0] > 0:
        if len(sentToken) == 1 and len(rcvdToken) == 1:
            ...
            r = records.TraderTransaction(
                txn, timestamp, sentToken[0], rcvdToken[0], sentAmount[0], rcvdAmount[0]
            )
            fiatValues = getSwapFiatValues(
                timestamp, sentToken[0], sentAmount[0], rcvdToken[0], rcvdAmount[0]
            )
            ...
            logging.error(
                "wtf swap: {0}: sent-{1} rcvd-{2}".format(
                    txn, str(sentToken), str(rcvdToken)
                )
            )
        else:
            ...
            if len(sentToken) == 1 and len(rcvdToken) == 2:
                ...
                logging.info(
                    "Liquidity withdraw event {2} send {0} rcvd {1}".format(
                        str(sentToken), str(rcvdToken), txn
                    )
                )
                r = records.LiquidityTransaction(
                    txn,
                    timestamp,
                    "withdraw",
                    sentToken[0],
                ...
                fiatValues = getSwapFiatValues(
                    timestamp, rcvdToken[0], rcvdAmount[0], rcvdToken[1], rcvdAmount[1]
                )
                ...
                logging.info(
                    "Liquidity deposit event {2} send {0} rcvd {1}".format(
                        str(sentToken), str(rcvdToken), txn
                    )
                )
                r = records.LiquidityTransaction(
                ...
                fiatValues = getSwapFiatValues(
                    timestamp, sentToken[0], sentAmount[0], sentToken[1], sentAmount[1]
                )
                r.coin1FiatValue = fiatValues[0]
                r.coin2FiatValue = fiatValues[1]
                return r
...
def extractSummonResults(w3, txn, account, timestamp, receipt, network):
    ...
    for log in decoded_logs:
        logging.debug(
            "{3} transfer for summon from: {0} to: {1} value: {2}".format(
                log["args"]["from"],
                log["args"]["to"],
                log["args"]["value"],
    ...
    assistant = "unk"
    contract = w3.eth.contract(
        address="0x65DEA93f7b886c33A78c10343267DD39727778c2", abi=ABI
    )
    decoded_logs = contract.events.CrystalSummoned().processReceipt(
        receipt, errors=DISCARD
    )
    for log in decoded_logs:
        ...
        if type(log["args"]["generation"]) is int:
            ...
            rc = records.TavernTransaction(
                txn,
                "hero",
                "/".join((str(summoner), str(assistant))),
                "summon",
                timestamp,
                powerToken,
                jewelAmount,
            ...
            logging.info(
                "{3} Summon Crystal event {0} jewel/{1} tears {2} gen result".format(
                    jewelAmount, tearsAmount, log["args"]["generation"], txn
                )
            )

    decoded_logs = contract.events.AuctionSuccessful().processReceipt(
        receipt, errors=DISCARD
    )
    for log in decoded_logs:
        ...
        if hiredFromAccount != "":
            ...
            if assistant != "unk":
                hiredHero = assistant
            rs = records.TavernTransaction(
                txn, "hero", hiredHero, "hire", timestamp, powerToken, hiringProceeds
            )
    ...
    for log in decoded_logs:
        logging.info(
            "{3} crystal open {0} hero {1} for {2}".format(
                log["args"]["crystalId"],
                log["args"]["heroId"],
                log["args"]["owner"],
                txn,
...
def extractMeditationResults(w3, txn, account, timestamp, receipt):
    ...
    for log in decoded_logs:
        if log["address"] in [
            ...
            runeCost += (
                prices.priceLookup(timestamp, log["address"]) * log["args"]["value"]
            )

    ABI = contracts.getABI("MeditationCircle")
    contract = w3.eth.contract(
        address="0x0594D86b2923076a2316EaEA4E1Ca286dAA142C1", abi=ABI
    )
    complete_logs = contract.events.MeditationBegun().processReceipt(
        receipt, errors=DISCARD
    )
    r = None
    rs = None
    ...
    for log in complete_logs:
        ...
        if type(heroID) is int:
            rs = records.TavernTransaction(
                txn, "hero", heroID, "levelup", timestamp, ptAddress, ptAmount
            )
            rs.fiatAmount = prices.priceLookup(timestamp, rs.coinType) * rs.coinCost
            r = records.TavernTransaction(
                txn, "hero", heroID, "meditate", timestamp, runeAddress, int(runeAmount)
            )
            r.fiatAmount = runeCost
            if crystal != "0x0000000000000000000000000000000000000000":
                rt = records.TavernTransaction(
                    txn, "hero", heroID, "enhance", timestamp, crystal, 1
                )
                rt.fiatAmount = prices.priceLookup(timestamp, crystal)
            logging.info(
                "{3} Meditation event {0} power token/{1} runes {2} heroid".format(
                    ptAmount, runeAmount, heroID, txn
                )
            )
    return [r, rs, rt]
...
def extractAuctionResults(w3, txn, account, timestamp, receipt, auctionType):
    ...
    for log in decoded_logs:
        logging.debug(
            "Jewel transfer for auction from: {0} to: {1} value: {2}".format(
                log["args"]["from"], log["args"]["to"], log["args"]["value"]
            )
        )
        if log["args"]["to"] not in contracts.tx_fee_targets:
    ...
    contract = w3.eth.contract(
        address="0x13a65B9F8039E2c032Bc022171Dc05B30c3f2892", abi=ABI
    )
    decoded_logs = contract.events.AuctionSuccessful().processReceipt(
        receipt, errors=DISCARD
    )
    ...
    for log in decoded_logs:
        ...
        logging.info(
            "  {2}  Bought {3} {0} for {1} jewel".format(
                log["args"]["tokenId"], auctionPrice, log["args"]["winner"], auctionType
            )
        )
        r = records.TavernTransaction(
            txn,
        ...
        if auctionSeller != "":
            logging.info(
                "  {2}  Sold {3} {0} for {1} jewel".format(
                    log["args"]["tokenId"], auctionPrice, auctionSeller, auctionType
                )
            )
            rs = records.TavernTransaction(
                txn,
        ...
        if "to" in log["args"] and "from" in log["args"]:
            if log["args"]["to"] == account:
                ...
                if log["address"] in rcvdTokens:
                    rcvdTokens[log["address"]] += contracts.valueFromWei(
                        log["args"]["value"], log["address"]
                    )
                else:
                    rcvdTokens[log["address"]] = contracts.valueFromWei(
                        log["args"]["value"], log["address"]
                ...
                logging.info(
                    "ignored airdrop log {0} to {1} not involving account".format(
                        log["args"]["from"], log["args"]["to"]
                    )
                )
...
def extractQuestResults(w3, txn, timestamp, receipt, address):
    ...
    if address in [
        ...
        contract = w3.eth.contract(
            address="0xAa9a289ce0565E4D6548e63a441e7C084E6B52F6", abi=ABI
        )
        ...
        contract = w3.eth.contract(
            address="0x5100Bd31b822371108A0f63DCFb6594b9919Eaf4", abi=ABI
        )
        v2_logs = contract.events.QuestReward().processReceipt(receipt, errors=DISCARD)
    rewardTotals = {}
    txns = []
    for log in v3_logs:
        if (
            "amount" in log["args"]
            and "reward" in log["args"]
            and log["args"]["reward"] != "0x0000000000000000000000000000000000000000"
        ):
            # Keep a running total of each unique reward item in this quest result
            rewardQuantity = contracts.valueFromWei(
                log["args"]["amount"], log["args"]["reward"]
            )
            logging.info(
                "    Hero {2} on quest {3} got reward of {0} {1}".format(
                    rewardQuantity,
                    contracts.getAddressName(log["args"]["reward"]),
                    log["args"]["heroId"],
                    log["args"]["questId"],
                )
            )
    ...
    for log in v2_logs:
        if (
            "itemQuantity" in log["args"]
            and "rewardItem" in log["args"]
            and log["args"]["rewardItem"]
            != "0x0000000000000000000000000000000000000000"
        ):
            # Keep a running total of each unique reward item in this quest result
            rewardQuantity = contracts.valueFromWei(
                log["args"]["itemQuantity"], log["args"]["rewardItem"]
            )
            if log["args"]["rewardItem"] in contracts.address_map:
                logging.info(
                    "    Hero {2} on quest {3} got reward of {0} {1}".format(
                        rewardQuantity,
                        contracts.getAddressName(log["args"]["rewardItem"]),
                        log["args"]["heroId"],
                        log["args"]["questId"],
                    )
                )
                ...
                logging.info(
                    "    Hero {2} on quest {3} got reward of {0} unknown({1})".format(
                        rewardQuantity,
                        log["args"]["rewardItem"],
                        log["args"]["heroId"],
                        log["args"]["questId"],
...
def extractAlchemistResults(w3, txn, account, timestamp, receipt):
    ...
    for log in decoded_logs:
        ...
        if "to" in log["args"] and "from" in log["args"]:
            if log["args"]["to"] == account:
                ...
                logging.debug(
                    "ignored alchemist log {0} to {1} not involving account".format(
                        log["args"]["from"], log["args"]["to"]
                    )
                )
    ...
    ingredientValue = 0
    for i in range(len(sentToken)):
        ingredientList += "{0}x{1}, ".format(
            contracts.address_map[sentToken[i]], sentAmount[i]
        )
    ...
    if len(rcvdToken) == 1:
        ...
        logging.error(
            "Failed to add alchemist record, wrong token rcvd {0}".format(txn)
        )
    return r
...
def extractPotionResults(w3, txn, account, timestamp, receipt, inputs):
    ...
    contract = w3.eth.contract(
        address="0x38e76972BD173901B5E5E43BA5cB464293B80C31", abi=ABI
    )
    ...
    if consumedItem != "":
        r = records.TavernTransaction(
            txn, "hero", heroId, "consume", timestamp, consumedItem, consumedCount
        )
...
def extractJourneyResults(w3, txn, account, timestamp, receipt, inputs):
    ...
    contract = w3.eth.contract(
        address="0xE92Db3bb6E4B21a8b9123e7FdAdD887133C64bb7", abi=ABI
    )
    input_data = contract.decode_function_input(inputs)
    logging.info(str(input_data))
    rewards = []
    ...
    for log in decoded_logs:
        ...
        if log["args"]["player"] == account and log["args"]["heroSurvived"] == False:
            perishedHeroRewards[log["args"]["heroId"]] = {
                "0x72Cb10C6bfA5624dD07Ef608027E366bd690048F": contracts.valueFromWei(
                    log["args"]["jewelAmount"],
                    "0x72Cb10C6bfA5624dD07Ef608027E366bd690048F",
                )
            }
    # JourneyReward event contains other rewards runes/stones/crystals
    decoded_logs = contract.events.JourneyReward().processReceipt(
        receipt, errors=DISCARD
    )
    for log in decoded_logs:
        logging.info(log)
        if log["args"]["heroId"] in perishedHeroRewards:
            perishedHeroRewards[log["args"]["heroId"]][log["args"]["rewardItem"]] = log[
                "args"
            ]["itemQuantity"]
...
def extractHatchingResults(w3, txn, account, timestamp, receipt):
    ...
    for log in decoded_logs:
        logging.debug(
            "{3} transfer for incubate from: {0} to: {1} value: {2}".format(
                log["args"]["from"],
                log["args"]["to"],
                log["args"]["value"],
                contracts.getAddressName(log["address"]),
        ...
        if log["address"] == powerToken:
            ...
            otherCosts[log["address"]] = contracts.valueFromWei(
                log["args"]["value"], log["address"]
            )
    ...
    contract = w3.eth.contract(
        address="0x576C260513204392F0eC0bc865450872025CB1cA", abi=ABI
    )
    incubate_logs = contract.events.EggIncubated().processReceipt(
        receipt, errors=DISCARD
    )
    ...
    for log in incubate_logs:
        ...
        if type(log["args"]["eggId"]) is int:
            ...
            r.fiatAmount = prices.priceLookup(timestamp, r.coinType) * r.coinCost
            logging.info(
                "{2} Incubate Egg event {0} jewel {1} type {3} tier".format(
                    jewelAmount, log["args"]["eggType"], txn, log["args"]["tier"]
                )
            )
    ...
    for log in crack_logs:
        if type(log["args"]["eggId"]) is int:
            eggId = log["args"]["eggId"]
            r = records.TavernTransaction(
                txn, "pet", log["args"]["petId"], "crack", timestamp, "", 0
            )
            logging.info(
                "{3} egg crack {0} pet {1} for {2}".format(
                    log["args"]["eggId"],
                    log["args"]["petId"],
                    log["args"]["owner"],
                    txn,
...
def extractPetBurnResults(w3, txn, account, timestamp, receipt):
    ...
    contract = w3.eth.contract(
        address="0xeaF833A0Ae97897f6F69a728C9c17916296cecCA", abi=ABI
    )
    r1 = None
    r2 = None
    # create one record for each pet burned with egg reward on first one
    decoded_logs = contract.events.PetExchangeCompleted().processReceipt(
        receipt, errors=DISCARD
    )
    for log in decoded_logs:
        r1 = records.TavernTransaction(
            txn,
            "pet",
            log["args"]["eggId1"],
            "perished",
            timestamp,
            constants.EGG_TYPES[log["args"]["eggTypeRecieved"]],
            1,
        )
        r1.fiatAmount = prices.priceLookup(
            timestamp, constants.EGG_TYPES[log["args"]["eggTypeRecieved"]]
        )
        r2 = records.TavernTransaction(
            txn, "pet", log["args"]["eggId2"], "perished", timestamp, "", 0
        )
...
def extractDFKDuelResults(w3, txn, account, timestamp, receipt, inputs):
    ...
    for log in decoded_logs:
        ...
        if "to" in log["args"] and "from" in log["args"]:
            if (
                log["args"]["to"] == account
                and log["address"] == "0x0405f1b828C7C9462877cC70A9f266887FF55adA"
            ):
                rcvdToken.append(log["address"])
                rcvdAmount.append(
                    contracts.valueFromWei(log["args"]["value"], log["address"])
                ...
                and log["address"] == "0x72Cb10C6bfA5624dD07Ef608027E366bd690048F"
            ):
                sentToken.append(log["address"])
                sentAmount.append(
                    contracts.valueFromWei(log["args"]["value"], log["address"])
                ...
                logging.debug(
                    "ignored duel log {0} to {1} not involving account".format(
                        log["args"]["from"], log["args"]["to"]
                    )
                )
    ...
    contract = w3.eth.contract(
        address="0xE97196f4011dc9DA0829dd8E151EcFc0f37EE3c7", abi=ABI
    )
    input_data = contract.decode_function_input(inputs)
    logging.info(str(input_data))
    ...
    if len(rcvdToken) > 0:
        ...
        decoded_logs = contract.events.DuelEntryCreated().processReceipt(
            receipt, errors=DISCARD
        )
        for log in decoded_logs:
            entryId = log["args"]["id"]
        r = records.TavernTransaction(
            txn, "hero", entryId, "pvpfee", timestamp, sentToken[0], sentAmount[0]
        )
...
def extractBridgeResults(w3, txn, account, timestamp, receipt):
    ...
    for log in decoded_logs:
        if (
            ...
            logging.info(
                "{3} ignoring token transfer not from/to account from {0} to {1} value {2}".format(
                    log["args"]["from"], log["args"]["to"], log["args"]["value"], txn
                )
            )
            continue
        ...
        if tokenValue > 0:
            logging.info(
                "{3} wallet bridge from: {0} to: {1} value: {2}".format(
                    log["args"]["from"], log["args"]["to"], tokenValue, tokenName
                )
            )
...
def extractLendingResults(w3, txn, account, timestamp, receipt, network, value):
    ...
    contract = w3.eth.contract(
        address="0xCa3e902eFdb2a410C952Fd3e4ac38d7DBDCB8E96", abi=ABI
    )
    decoded_logs = contract.events.Transfer().processReceipt(receipt, errors=DISCARD)
    for log in decoded_logs:
        if log["args"]["from"] == account:
    ...
    if sentToken == "" and value > 0:
        ...
        contract = w3.eth.contract(
            address="0x34B9aa82D89AE04f0f546Ca5eC9C93eFE1288940", abi=ABI
        )
    ...
    for log in decoded_logs:
        logging.info(
            "Pay back {0} of borrow, remaining borrow is now {1} {2} total borrows {3}".format(
                contracts.valueFromWei(log["args"]["repayAmount"], sentToken),
                contracts.valueFromWei(log["args"]["accountBorrows"], sentToken),
                contracts.getAddressName(log["address"]),
                contracts.valueFromWei(log["args"]["totalBorrows"], sentToken),
            )
        )
        r = records.LendingTransaction(
            txn, timestamp, "repay", log["address"], sentToken, sentValue
        )
    ...
    for log in decoded_logs:
        logging.info(
            "Borrowed {0} {1} account borrowed is now {2} total borrowed is {3}".format(
                contracts.valueFromWei(log["args"]["borrowAmount"], rcvdToken),
                contracts.getAddressName(log["address"]),
                contracts.valueFromWei(log["args"]["accountBorrows"], rcvdToken),
                contracts.valueFromWei(log["args"]["totalBorrows"], rcvdToken),
            )
        )
        r = records.LendingTransaction(
            txn,
        ...
        r.fiatValue = prices.priceLookup(timestamp, rcvdToken) * contracts.valueFromWei(
            log["args"]["borrowAmount"], rcvdToken
        )
    ...
    for log in decoded_logs:
        logging.info(
            "Redeemed {0} {1} for {2} tq tokens".format(
                contracts.valueFromWei(log["args"]["redeemAmount"], rcvdToken),
                contracts.getAddressName(log["address"]),
                log["args"]["redeemTokens"],
            )
        )
        r = records.LendingTransaction(
            txn,
        ...
        r.fiatValue = prices.priceLookup(timestamp, rcvdToken) * contracts.valueFromWei(
            log["args"]["redeemAmount"], rcvdToken
        )
    ...
    for log in decoded_logs:
        logging.info(
            "Lended {0} {1} rcvd {2} tq tokens".format(
                contracts.valueFromWei(log["args"]["mintAmount"], sentToken),
                contracts.getAddressName(log["address"]),
                log["args"]["mintTokens"],
            )
        )
        r = records.LendingTransaction(
            txn, timestamp, "lend", log["address"], sentToken, sentValue
        )
        r.fiatValue = prices.priceLookup(timestamp, sentToken) * sentValue
    decoded_logs = contract.events.LiquidateBorrow().processReceipt(
        receipt, errors=DISCARD
    )
    for log in decoded_logs:
        logging.info(
            "Liquidate {0} {3} of borrow, seized {1} {2}".format(
                contracts.valueFromWei(log["args"]["repayAmount"], sentToken),
                contracts.valueFromWei(
                    log["args"]["seizeTokens"], log["args"]["tqTokenCollateral"]
                ),
                contracts.getAddressName(log["args"]["tqTokenCollateral"]),
                contracts.getAddressName(rcvdToken),
            )
        )
        r = records.LendingTransaction(
            txn, timestamp, "liquidate", log["address"], sentToken, sentValue
        )
        r.fiatValue = prices.priceLookup(timestamp, sentToken) * sentValue
