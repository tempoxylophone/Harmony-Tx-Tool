...
def costBasisSort(eventList, costBasis):
    if costBasis in ['fifo', 'acb']:
        ...
        return sorted(eventList, key=lambda x: x.fiatReceiveValue / x.receiveAmount if x.receiveAmount else 0,
                      reverse=True)
    else:
        return eventList
...
def buildTavernRecords(tavernEvents, startDate, endDate):
            ...
            if event.itemType == 'land':
                if event.itemID in landExpenses:
                    if event.event in landExpenses[event.itemID].description:
                        landExpenses[event.itemID].description = landExpenses[event.itemID].description.replace(
                            event.event, '{0}+'.format(event.event))
                    else:
                        landExpenses[event.itemID].description += ''.join((' ', event.event))
                    landExpenses[event.itemID].costs += event.fiatAmount
                    if landExpenses[event.itemID].acquiredDate == None or eventDate < landExpenses[
                        event.itemID].acquiredDate:
                    ...
                    ti = TaxItem(event.txHash, event.coinCost, contracts.getAddressName(event.coinType), 0, '',
                                 '{2} {0} {1}'.format(event.itemID, event.event, event.itemType), 'expenses', None,
                                 event.fiatType, 0, eventDate, event.fiatAmount)
                    if hasattr(event, 'fiatFeeValue'):
                ...
                if event.itemID in petExpenses:
                    if event.event in petExpenses[event.itemID].description:
                        petExpenses[event.itemID].description = petExpenses[event.itemID].description.replace(
                            event.event, '{0}+'.format(event.event))
                    else:
                        petExpenses[event.itemID].description += ''.join((' ', event.event))
                    petExpenses[event.itemID].costs += event.fiatAmount
                    if petExpenses[event.itemID].acquiredDate == None or eventDate < petExpenses[
                        event.itemID].acquiredDate:
                ...
                if event.itemID in heroExpenses:
                    if event.event in heroExpenses[event.itemID].description:
                        heroExpenses[event.itemID].description = heroExpenses[event.itemID].description.replace(
                            event.event, '{0}+'.format(event.event))
                    else:
                        heroExpenses[event.itemID].description += ''.join((' ', event.event))
                    heroExpenses[event.itemID].costs += event.fiatAmount
                    if heroExpenses[event.itemID].acquiredDate == None or eventDate < heroExpenses[
                        event.itemID].acquiredDate:
        ...
        if event.event in ['hire'] and eventDate >= startDate and eventDate <= endDate:
            if event.itemID in heroIncome:
                if event.event in heroIncome[event.itemID].description:
                    heroIncome[event.itemID].description = heroIncome[event.itemID].description.replace(event.event,
                                                                                                        '{0}+'.format(
                                                                                                            event.event))
                else:
                ...
                ti = TaxItem(event.txHash, 0, '', event.coinCost, contracts.getAddressName(event.coinType),
                             'Hero {0} {1}'.format(event.itemID, event.event), 'income', eventDate, event.fiatType,
                             event.fiatAmount)
    ...
    for event in tavernEvents:
        ...
        if event.event == 'perished' and eventDate >= startDate and eventDate <= endDate:
            if event.itemID in perishRewards:
                ...
                ti = TaxItem(event.txHash, 0, '', event.coinCost, contracts.getAddressName(event.coinType),
                             'Perished {1} {0}'.format(event.itemID, event.itemType), 'gains', eventDate,
                             event.fiatType, event.fiatAmount)
                if hasattr(event, 'fiatFeeValue'):
                    ti.txFees = event.fiatFeeValue
    ...
    for event in tavernEvents:
        ...
        if event.event == 'sale' and eventDate >= startDate and eventDate <= endDate:
            ti = TaxItem(event.txHash, 0, '', event.coinCost, contracts.getAddressName(event.coinType),
                         'Sold {1} {0}'.format(event.itemID, event.itemType), 'gains', eventDate, event.fiatType,
                         event.fiatAmount)
            if hasattr(event, 'fiatFeeValue'):
                ti.txFees = event.fiatFeeValue
            ti.amountNotAccounted = 1
...
def buildSwapRecords(swapEvents, startDate, endDate, walletEvents, airdropEvents, gardensEvents, questEvents,
                     tavernEvents, lendingEvents, costBasis, purchaseAddresses):
    results = []
    ...
    for item in walletEvents:
        if (item.action == 'withdraw' and item.address in purchaseAddresses) or item.action == 'donation':
            si = records.TraderTransaction(item.txHash, item.timestamp, item.coinType, 'fiat value', item.coinAmount,
                                           item.fiatValue)
    ...
    for sEvent in swapEvents:
        ci = CostBasisItem(sEvent.txHash, sEvent.timestamp, sEvent.receiveType, sEvent.receiveAmount, sEvent.fiatType,
                           sEvent.fiatReceiveValue)
        cbList.append(ci)
    for aEvent in airdropEvents:
        ...
        ci = CostBasisItem(aEvent.txHash, aEvent.timestamp, aEvent.tokenReceived, aEvent.tokenAmount, aEvent.fiatType,
                           aEvent.fiatValue)
        cbList.append(ci)
    for gEvent in gardensEvents:
        if not hasattr(gEvent, 'amountNotAccounted'):
            gEvent.amountNotAccounted = gEvent.coinAmount
        ci = CostBasisItem(gEvent.txHash, gEvent.timestamp, gEvent.coinType, gEvent.coinAmount, gEvent.fiatType,
                           gEvent.fiatValue)
        cbList.append(ci)
    for qEvent in questEvents:
        if not hasattr(qEvent, 'amountNotAccounted'):
            qEvent.amountNotAccounted = qEvent.rewardAmount
        ci = CostBasisItem(qEvent.txHash, qEvent.timestamp, qEvent.rewardType, qEvent.rewardAmount, qEvent.fiatType,
                           qEvent.fiatValue)
        cbList.append(ci)
    for tEvent in tavernEvents:
        if tEvent.event in ['hire', 'sale', 'perished']:
            ci = CostBasisItem(tEvent.txHash, tEvent.timestamp, tEvent.coinType, tEvent.coinCost, tEvent.fiatType,
                               tEvent.fiatAmount)
            cbList.append(ci)
    for lEvent in lendingEvents:
        if lEvent.event in ['borrow']:
            ci = CostBasisItem(lEvent.txHash, lEvent.timestamp, lEvent.coinType, lEvent.coinAmount, lEvent.fiatType,
                               lEvent.fiatValue)
            cbList.append(ci)
    ...
    for wEvent in walletEvents:
        if wEvent.action in ['deposit', 'payment']:
            ci = CostBasisItem(wEvent.txHash, wEvent.timestamp, wEvent.coinType, wEvent.coinAmount, wEvent.fiatType,
                               wEvent.fiatValue)
            cbList.append(ci)
    ...
    for event in swapEvents:
        ...
        if event.swapType in contracts.gold_values and event.receiveType in [
            '0x3a4EDcf3312f44EF027acfd8c21382a5259936e7', '0x576C260513204392F0eC0bc865450872025CB1cA']:
            continue
        eventDate = datetime.date.fromtimestamp(event.timestamp)
        if eventDate >= startDate and eventDate <= endDate:
            if event.receiveType == 'fiat value':
                ...
                actionStr = 'Sold'
            ti = TaxItem(event.txHash, event.swapAmount, contracts.getAddressName(event.swapType), event.receiveAmount,
                         contracts.getAddressName(event.receiveType),
                         '{4} {0:.5f} {1} for {2:.5f} {3}'.format(event.swapAmount,
                                                                  contracts.getAddressName(event.swapType),
                                                                  event.receiveAmount,
                                                                  contracts.getAddressName(event.receiveType),
                                                                  actionStr), 'gains', eventDate, event.fiatType,
                         event.fiatSwapValue)
            if hasattr(event, 'fiatFeeValue'):
                ti.txFees = event.fiatFeeValue
                    ...
                    if costBasis == 'acb':
                        if searchEvent.receiveAmountNotAccounted > 0:
                            ...
                            if hasattr(searchEvent, 'fiatFeeValue'):
                                acbValue += (searchEvent.fiatReceiveValue + searchEvent.fiatFeeValue) * Decimal(
                                    searchEvent.receiveAmountNotAccounted / searchEvent.receiveAmount)
                            else:
                                acbValue += searchEvent.fiatReceiveValue * Decimal(
                                    searchEvent.receiveAmountNotAccounted / searchEvent.receiveAmount)
                        ...
                        if searchEvent.receiveAmountNotAccounted <= event.swapAmountNotAccounted:
                            ...
                            ti.costs += searchEvent.fiatReceiveValue * Decimal(
                                searchEvent.receiveAmountNotAccounted / searchEvent.receiveAmount)
                            event.swapAmountNotAccounted -= searchEvent.receiveAmountNotAccounted
                            searchEvent.receiveAmountNotAccounted = 0
                        else:
                            ...
                            ti.costs += Decimal(
                                searchEvent.fiatReceiveValue / searchEvent.receiveAmount) * event.swapAmountNotAccounted
                            searchEvent.receiveAmountNotAccounted -= event.swapAmountNotAccounted
                            event.swapAmountNotAccounted = 0
                            break
    ...
    for event in liquidityEvents:
        ...
        if event.action == 'withdraw' and eventDate >= startDate and eventDate <= endDate:
            ti = TaxItem(event.txHash, event.poolAmount, contracts.getAddressName(event.poolAddress),
                         '{0}/{1}'.format(event.coin1Amount, event.coin2Amount),
                         '{0}/{1}'.format(contracts.getAddressName(event.coin1Type),
                                          contracts.getAddressName(event.coin2Type)),
                         'Liquidity Withdrawal {0}'.format(contracts.getAddressName(event.poolAddress)), 'gains',
                         eventDate, event.fiatType, event.coin1FiatValue + event.coin2FiatValue)
            if hasattr(event, 'fiatFeeValue'):
                ti.txFees = event.fiatFeeValue
            ...
            for searchEvent in liquidityEvents:
                ...
                if searchEvent.action == 'deposit' and searchEvent.poolAddress == event.poolAddress and searchEvent.timestamp <= event.timestamp:
                    ...
                    if searchEvent.amountNotAccounted <= event.amountNotAccounted:
                        ...
                        ti.costs += searchEventTotalValue * (searchEvent.amountNotAccounted / (
                                searchEvent.coin1Amount + searchEvent.coin2Amount))
...
def buildBankRecords(bankEvents, startDate, endDate):
    ...
    for event in bankEvents:
        ...
        if event.action == 'withdraw' and eventDate >= startDate and eventDate <= endDate:
            ti = TaxItem(event.txHash, 0, '', event.coinAmount, contracts.getAddressName(event.coinType),
                         'Bank Rewards {0}'.format(contracts.getAddressName(event.coinType)), 'income', eventDate,
                         event.fiatType, event.fiatValue)
            if hasattr(event, 'fiatFeeValue'):
                ti.txFees = event.fiatFeeValue
            ...
            for searchEvent in bankEvents:
                ...
                if searchEvent.action == 'deposit' and searchEvent.coinType == event.coinType and searchEvent.timestamp <= event.timestamp:
                    ...
                    if searchEvent.amountNotAccounted <= event.amountNotAccounted:
                        # deposit is smaller than this event, so use it all up as cost basis
                        ti.costs += (jewelPrice * searchEvent.coinAmount) * (
                                searchEvent.amountNotAccounted / (searchEvent.coinAmount / searchEvent.xRate)) * (
                                            searchEvent.xRate / event.xRate)
                        event.amountNotAccounted -= searchEvent.amountNotAccounted
                        searchEvent.amountNotAccounted = 0
                    else:
                        # deposit is larger than remaining cost basis we need to find so use part of it
                        ti.costs += ((
                                             jewelPrice * searchEvent.coinAmount) / searchEvent.amountNotAccounted) * event.amountNotAccounted * (
                                            searchEvent.xRate / event.xRate)
                        event.amountNotAccounted = 0
                        searchEvent.amountNotAccounted -= event.amountNotAccounted
            # Note any amount of cost basis not found for later red flag
    ...
    for event in gardensEvents:
        ...
        if event.event == 'staking-reward' and eventDate >= startDate and eventDate <= endDate:
            if ''.join((eventDate.strftime('%d-%m-%Y'), event.event, event.coinType)) in rewardGroups:
                rewardGroups[
                    ''.join((eventDate.strftime('%d-%m-%Y'), event.event, event.coinType))].proceeds += event.fiatValue
                # this is hacky, but I'm just gonna use it to keep track of total jewel since it is not needed
                rewardGroups[
                    ''.join((eventDate.strftime('%d-%m-%Y'), event.event, event.coinType))].costs += event.coinAmount
            else:
                ti = TaxItem(event.txHash, 0, '', event.coinAmount, contracts.getAddressName(event.coinType),
                             '{0} Staking Reward'.format(contracts.getAddressName(event.coinType)), 'income', eventDate,
                             event.fiatType, event.fiatValue)
                if hasattr(event, 'fiatFeeValue'):
                    ti.txFees = event.fiatFeeValue
                # Not really!
    ...
    for event in airdropEvents:
        ...
        if eventDate >= startDate and eventDate <= endDate:
            if event.tokenAmount % 1 > 0:
                desc = '{2} {0:.3f} {1}'.format(event.tokenAmount, contracts.getAddressName(event.tokenReceived),
                                                eventTitle)
            else:
                desc = '{2} {0} {1}'.format(int(event.tokenAmount), contracts.getAddressName(event.tokenReceived),
                                            eventTitle)
            ti = TaxItem(event.txHash, 0, '', event.tokenAmount, contracts.getAddressName(event.tokenReceived), desc,
                         'income', eventDate, event.fiatType, event.fiatValue)
            if hasattr(event, 'fiatFeeValue'):
                ti.txFees = event.fiatFeeValue
            ti.amountNotAccounted = 0
            results.append(ti)
...
def buildLendingRecords(lendingEvents, startDate, endDate):
    ...
    for event in lendingEvents:
        ...
        if event.event == 'interest' and eventDate >= startDate and eventDate <= endDate:
            ...
            ti = TaxItem(event.txHash, event.coinAmount, event.coinType, 0, '', desc, 'expenses', eventDate,
                         event.fiatType, 0, None, event.fiatValue)
            ti.amountNotAccounted = 0
            results.append(ti)
    return results
    ...
    for event in questEvents:
        ...
        if event.rewardType == '0x72Cb10C6bfA5624dD07Ef608027E366bd690048F' and eventDate >= startDate and eventDate <= endDate:
            if ''.join((eventDate.strftime('%d-%m-%Y'), event.rewardType)) in itemGroups:
                ...
                ti = TaxItem(event.txHash, 0, '', event.rewardAmount, contracts.getAddressName(event.rewardType),
                             'Quest Jewel Rewards', 'income', eventDate, event.fiatType, event.fiatValue)
                # Not really!
    ...
    for event in walletEvents:
        ...
        if event.action == 'payment' and eventDate >= startDate and eventDate <= endDate:
            if ''.join((eventDate.strftime('%d-%m-%Y'), event.coinType)) in itemGroups:
                ...
                ti = TaxItem(event.txHash, 0, '', event.coinAmount, contracts.getAddressName(event.coinType), 'Payment',
                             'income', eventDate, event.fiatType, event.fiatValue)
                if hasattr(event, 'fiatFeeValue'):
                    ti.txFees += event.fiatFeeValue
                # Not really!
            ...
            if ''.join((eventDate.strftime('%d-%m-%Y'), event.coinType)) in donationGroups:
                ...
                ti = TaxItem(event.txHash, event.coinAmount, contracts.getAddressName(event.coinType), 0, '',
                             'Charitable Donation', 'expenses', eventDate, event.fiatType, 0, None, event.fiatValue)
                if hasattr(event, 'fiatFeeValue'):
                    ti.txFees += event.fiatFeeValue
                # Not really!
