        ...
        if receipt['status'] == 1:
            logging.info(
                "{5}:{4} | {3}: {0} - {1} - {2}".format(action, '{:f} value'.format(value), '{:f} fee'.format(txFee),
                                                        tx, datetime.datetime.fromtimestamp(timestamp).isoformat(),
                                                        network))
            if 'Quest' in action and result['input'] != '0x':
                    ...
                    if results[1] != None and results[1].event == 'crack':
                        ...
                        if results[0] in petEggs:
                            ...
                            for k, v in petEggs[results[0]][6].items():
                                ...
                                logging.info(
                                    'Adding cost {0} {1} for {2}'.format(v, contracts.getAddressName(k), priceEach))
                ...
                crystalId = 0
                for log in decoded_logs:
                    logging.info('Purchase Gen0 Crystal id {0} price {1}'.format(log['args']['crystalId'],
                                                                                 log['args']['purchasePrice']))
                ...
                for log in decoded_logs:
                    logging.info(
                        'Open Gen0 Crystal id {0} heroid {1}'.format(log['args']['crystalId'], log['args']['heroId']))
                    crystalId = log['args']['crystalId']
                ...
                if crystalId != 0:
                    ...
                    if log['args']['crystalId'] not in heroCrystals:
                        ...
                        heroCrystals[log['args']['crystalId']][2] = max(purchasePrice,
                                                                        heroCrystals[log['args']['crystalId']][2])
                        heroCrystals[log['args']['crystalId']][3] = max(heroId,
                                                                        heroCrystals[log['args']['crystalId']][3])
                        if heroCrystals[log['args']['crystalId']][2] > 0 and heroCrystals[log['args']['crystalId']][
                            3] > 0:
                            heroPrice = Web3.fromWei(heroCrystals[log['args']['crystalId']][2], 'ether')
                            r = records.TavernTransaction(tx, 'hero', heroCrystals[log['args']['crystalId']][3],
                                                          'purchase', timestamp,
                                                          '0x72Cb10C6bfA5624dD07Ef608027E366bd690048F', heroPrice)
...
def extractBankResults(w3, txn, account, timestamp, receipt):
    ...
    for log in decoded_logs:
        ...
        if sentAmount > 0 and rcvdAmount > 0:
            if sentToken in ['0xA9cE83507D872C5e1273E745aBcfDa849DAA654F',
...
def extractGardenerResults(w3, txn, account, timestamp, receipt, network):
    ...
    for log in decoded_logs:
        # Token Transfers
        if 'to' in log['args'] and 'from' in log['args'] and log['address'] in contracts.address_map and 'LP Token' in \
                contracts.address_map[log['address']]:
            if log['args']['to'] == account:
...
def extractFarmResults(w3, txn, account, timestamp, receipt):
    ...
    for log in decoded_logs:
        # Token Transfers
        if 'to' in log['args'] and 'from' in log['args']:
            if ('Tranq' in contracts.getAddressName(log['address']) or 'Pangolin LP' in contracts.getAddressName(
                    log['address'])) and log['args']['to'] == account:
                farmEvent = 'withdraw'
            ...
            elif ('Tranq' in contracts.getAddressName(log['address']) or 'Pangolin LP' in contracts.getAddressName(
                    log['address'])) and log['args']['from'] == account:
                farmEvent = 'deposit'
                farmToken = log['address']
                farmAmount = Web3.fromWei(log['args']['value'], 'ether')
    if farmAmount > 0:
    ...
    for log in decoded_logs:
        ...
        if 'to' in log['args'] and 'from' in log['args']:
            ...
            if log['args']['to'] == account or (
                    log['address'] == '0xCCb93dABD71c8Dad03Fc4CE5559dC3D89F67a260' and log['args'][
                'from'] == '0x3C351E1afdd1b1BC44e931E12D4E05D6125eaeCa' and log['args'][
                ...
                rcvdAmount.append(contracts.valueFromWei(log['args']['value'], log['address']))
                    ...
                    log['address'] == '0xCCb93dABD71c8Dad03Fc4CE5559dC3D89F67a260' and log['args'][
                'to'] == '0x3C351E1afdd1b1BC44e931E12D4E05D6125eaeCa' and log['args'][
                        'from'] == '0x0000000000000000000000000000000000000000'):
                sentToken.append(log['address'])
                sentAmount.append(contracts.valueFromWei(log['args']['value'], log['address']))
            else:
                logging.debug(
                    'ignored swap log {0} to {1} not involving account'.format(log['args']['from'], log['args']['to']))
    ...
    if len(rcvdAmount) > 0 and rcvdAmount[0] > 0:
        if len(sentToken) == 1 and len(rcvdToken) == 1:
            ...
            if len(sentToken) == 1 and len(rcvdToken) == 2:
                ...
                logging.info(
                    'Liquidity withdraw event {2} send {0} rcvd {1}'.format(str(sentToken), str(rcvdToken), txn))
                r = records.LiquidityTransaction(txn, timestamp, 'withdraw', sentToken[0], sentAmount[0], rcvdToken[0],
                                                 rcvdAmount[0], rcvdToken[1], rcvdAmount[1])
                ...
                logging.info(
                    'Liquidity deposit event {2} send {0} rcvd {1}'.format(str(sentToken), str(rcvdToken), txn))
                r = records.LiquidityTransaction(txn, timestamp, 'deposit', rcvdToken[0], rcvdAmount[0], sentToken[0],
                                                 sentAmount[0], sentToken[1], sentAmount[1])
...
def extractSummonResults(w3, txn, account, timestamp, receipt, network):
    ...
    for log in decoded_logs:
        logging.debug(
            '{3} transfer for summon from: {0} to: {1} value: {2}'.format(log['args']['from'], log['args']['to'],
                                                                          log['args']['value'],
    ...
    for log in decoded_logs:
        ...
        if type(log['args']['generation']) is int:
            ...
            rc = records.TavernTransaction(txn, 'hero', '/'.join((str(summoner), str(assistant))), 'summon', timestamp,
                                           powerToken, jewelAmount)
            rc.fiatAmount = prices.priceLookup(timestamp, rc.coinType) * rc.coinCost
            r = records.TavernTransaction(txn, 'hero', '/'.join((str(summoner), str(assistant))), 'crystal', timestamp,
                                          tearsToken, int(tearsAmount))
            r.fiatAmount = prices.priceLookup(timestamp, r.coinType) * r.coinCost
            if bonusItem != '0x0000000000000000000000000000000000000000':
                rt = records.TavernTransaction(txn, 'hero', '/'.join((str(summoner), str(assistant))), 'enhance',
                                               timestamp, bonusItem, 1)
                rt.fiatAmount = prices.priceLookup(timestamp, bonusItem)
            logging.info('{3} Summon Crystal event {0} jewel/{1} tears {2} gen result'.format(jewelAmount, tearsAmount,
                                                                                              log['args']['generation'],
                                                                                              txn))
    ...
    decoded_logs = contract.events.CrystalOpen().processReceipt(receipt, errors=DISCARD)
    for log in decoded_logs:
        logging.info('{3} crystal open {0} hero {1} for {2}'.format(log['args']['crystalId'], log['args']['heroId'],
                                                                    log['args']['owner'], txn))
...
def extractMeditationResults(w3, txn, account, timestamp, receipt):
    ...
    decoded_logs = contract.events.Transfer().processReceipt(receipt, errors=DISCARD)
    for log in decoded_logs:
        if log['address'] in ['0x72Cb10C6bfA5624dD07Ef608027E366bd690048F',
    ...
    for log in complete_logs:
        ...
        if type(heroID) is int:
            ...
            logging.info(
                '{3} Meditation event {0} power token/{1} runes {2} heroid'.format(ptAmount, runeAmount, heroID, txn))
    return [r, rs, rt]
...
def extractAuctionResults(w3, txn, account, timestamp, receipt, auctionType):
    ...
    for log in decoded_logs:
        logging.debug(
            'Jewel transfer for auction from: {0} to: {1} value: {2}'.format(log['args']['from'], log['args']['to'],
                                                                             log['args']['value']))
        if log['args']['to'] not in contracts.tx_fee_targets:
    ...
    for log in decoded_logs:
        ...
        logging.info(
            "  {2}  Bought {3} {0} for {1} jewel".format(log['args']['tokenId'], auctionPrice, log['args']['winner'],
                                                         auctionType))
        r = records.TavernTransaction(txn, auctionType, log['args']['tokenId'], 'purchase', timestamp, auctionToken,
                                      auctionPrice)
        r.fiatAmount = prices.priceLookup(timestamp, auctionToken) * r.coinCost

        if auctionSeller != "":
            logging.info("  {2}  Sold {3} {0} for {1} jewel".format(log['args']['tokenId'], auctionPrice, auctionSeller,
                                                                    auctionType))
            rs = records.TavernTransaction(txn, auctionType, log['args']['tokenId'], 'sale', timestamp, auctionToken,
                                           sellerProceeds)
    ...
    for log in decoded_logs:
        ...
        if 'to' in log['args'] and 'from' in log['args']:
            if log['args']['to'] == account:
                ...
                logging.info('ignored airdrop log {0} to {1} not involving account'.format(log['args']['from'],
                                                                                           log['args']['to']))
...
def extractQuestResults(w3, txn, timestamp, receipt, address):
    ...
    txns = []
    for log in v3_logs:
        if 'amount' in log['args'] and 'reward' in log['args'] and log['args'][
            ...
            logging.info('    Hero {2} on quest {3} got reward of {0} {1}'.format(rewardQuantity,
                                                                                  contracts.getAddressName(
                                                                                      log['args']['reward']),
                                                                                  log['args']['heroId'],
                                                                                  log['args']['questId']))
            if log['args']['reward'] in rewardTotals:
                ...
                rewardTotals[log['args']['reward']] = rewardQuantity
    for log in v2_logs:
        if 'itemQuantity' in log['args'] and 'rewardItem' in log['args'] and log['args'][
            'rewardItem'] != '0x0000000000000000000000000000000000000000':
            # Keep a running total of each unique reward item in this quest result
            ...
            if log['args']['rewardItem'] in contracts.address_map:
                ...
                logging.info('    Hero {2} on quest {3} got reward of {0} unknown({1})'.format(rewardQuantity,
                                                                                               log['args'][
                                                                                                   'rewardItem'],
                                                                                               log['args']['heroId'],
                                                                                               log['args']['questId']))
    ...
    for log in decoded_logs:
        ...
        if 'to' in log['args'] and 'from' in log['args']:
            if log['args']['to'] == account:
                ...
                logging.debug('ignored alchemist log {0} to {1} not involving account'.format(log['args']['from'],
                                                                                              log['args']['to']))
    ...
    for log in decoded_logs:
        ...
        if log['args']['player'] == account and log['args']['heroSurvived'] == False:
            perishedHeroRewards[log['args']['heroId']] = {
                '0x72Cb10C6bfA5624dD07Ef608027E366bd690048F': contracts.valueFromWei(log['args']['jewelAmount'],
...
def extractHatchingResults(w3, txn, account, timestamp, receipt):
    ...
    for log in decoded_logs:
        logging.debug(
            '{3} transfer for incubate from: {0} to: {1} value: {2}'.format(log['args']['from'], log['args']['to'],
                                                                            log['args']['value'],
                                                                            contracts.getAddressName(log['address'])))
        if log['address'] == powerToken:
            jewelAmount += Web3.fromWei(log['args']['value'], 'ether')
    ...
    for log in incubate_logs:
        ...
        if type(log['args']['eggId']) is int:
            ...
            r = records.TavernTransaction(txn, 'pet', log['args']['eggType'], 'incubate', timestamp, powerToken,
                                          int(jewelAmount))
            r.fiatAmount = prices.priceLookup(timestamp, r.coinType) * r.coinCost
            logging.info(
                '{2} Incubate Egg event {0} jewel {1} type {3} tier'.format(jewelAmount, log['args']['eggType'], txn,
                                                                            log['args']['tier']))
    ...
    for log in crack_logs:
        if type(log['args']['eggId']) is int:
            ...
            logging.info('{3} egg crack {0} pet {1} for {2}'.format(log['args']['eggId'], log['args']['petId'],
                                                                    log['args']['owner'], txn))
...
def extractPetBurnResults(w3, txn, account, timestamp, receipt):
    ...
    for log in decoded_logs:
        r1 = records.TavernTransaction(txn, 'pet', log['args']['eggId1'], 'perished', timestamp,
                                       constants.EGG_TYPES[log['args']['eggTypeRecieved']], 1)
...
def extractDFKDuelResults(w3, txn, account, timestamp, receipt, inputs):
    ...
    for log in decoded_logs:
        ...
        if 'to' in log['args'] and 'from' in log['args']:
            if log['args']['to'] == account and log['address'] == '0x0405f1b828C7C9462877cC70A9f266887FF55adA':
                ...
                logging.debug(
                    'ignored duel log {0} to {1} not involving account'.format(log['args']['from'], log['args']['to']))
    ...
    if len(rcvdToken) > 0:
        r = records.TavernTransaction(txn, 'hero', input_data[1]['_duelId'], 'pvpreward', timestamp, rcvdToken[0],
                                      rcvdAmount[0])
        r.fiatValue = prices.priceLookup(timestamp, rcvdToken[0])
...
def extractBridgeResults(w3, txn, account, timestamp, receipt):
    ...
    for log in decoded_logs:
        ...
        if tokenValue > 0:
            logging.info('{3} wallet bridge from: {0} to: {1} value: {2}'.format(log['args']['from'], log['args']['to'],
...
def extractLendingResults(w3, txn, account, timestamp, receipt, network, value):
    ...
    for log in decoded_logs:
        logging.info('Pay back {0} of borrow, remaining borrow is now {1} {2} total borrows {3}'.format(
            contracts.valueFromWei(log['args']['repayAmount'], sentToken),
            contracts.valueFromWei(log['args']['accountBorrows'], sentToken), contracts.getAddressName(log['address']),
    ...
    for log in decoded_logs:
        logging.info('Borrowed {0} {1} account borrowed is now {2} total borrowed is {3}'.format(
            contracts.valueFromWei(log['args']['borrowAmount'], rcvdToken), contracts.getAddressName(log['address']),
            contracts.valueFromWei(log['args']['accountBorrows'], rcvdToken),
            contracts.valueFromWei(log['args']['totalBorrows'], rcvdToken)))
        r = records.LendingTransaction(txn, timestamp, 'borrow', log['address'], rcvdToken,
                                       contracts.valueFromWei(log['args']['borrowAmount'], rcvdToken))
        r.fiatValue = prices.priceLookup(timestamp, rcvdToken) * contracts.valueFromWei(log['args']['borrowAmount'],
                                                                                        rcvdToken)
    ...
    for log in decoded_logs:
        logging.info(
            'Redeemed {0} {1} for {2} tq tokens'.format(contracts.valueFromWei(log['args']['redeemAmount'], rcvdToken),
                                                        contracts.getAddressName(log['address']),
                                                        log['args']['redeemTokens']))
        r = records.LendingTransaction(txn, timestamp, 'redeem', log['address'], rcvdToken,
                                       contracts.valueFromWei(log['args']['redeemAmount'], rcvdToken))
        r.fiatValue = prices.priceLookup(timestamp, rcvdToken) * contracts.valueFromWei(log['args']['redeemAmount'],
                                                                                        rcvdToken)
    ...
    for log in decoded_logs:
        logging.info(
            'Lended {0} {1} rcvd {2} tq tokens'.format(contracts.valueFromWei(log['args']['mintAmount'], sentToken),
                                                       contracts.getAddressName(log['address']),
                                                       log['args']['mintTokens']))
    ...
    for log in decoded_logs:
        logging.info('Liquidate {0} {3} of borrow, seized {1} {2}'.format(
            contracts.valueFromWei(log['args']['repayAmount'], sentToken),
            contracts.valueFromWei(log['args']['seizeTokens'], log['args']['tqTokenCollateral']),
            contracts.getAddressName(log['args']['tqTokenCollateral']), contracts.getAddressName(rcvdToken)))
